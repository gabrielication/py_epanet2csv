...
def load_dataset(complete_path, cols, complete_path_stat, scaling=False, pairplot=False):
    ...
    ###########
    ########### USE DATAFRAME shaped
    ###########
    # features = data_scaled[cols].copy()
    # train_features = pd.DataFrame()
    # test_features = pd.DataFrame()
    # for jj in range(int(len(features)/83)):
    #     currentFlow = pd.DataFrame()
    #     for ii in range(83):
    #         currentFlowNodes = features.loc[[ii+(jj*83)]]
    #         currentFlowNodes.reset_index(inplace=True, drop=True)
    #         currentFlowNodes.rename(columns={'pressure_value': 'pressure_value_'+str(ii+1)}, inplace=True)
    #         currentFlowNodes.rename(columns={'base_demand': 'base_demand_'+str(ii+1)}, inplace=True)
    #         currentFlow = pd.concat([currentFlow, currentFlowNodes], axis=1)
    #     if jj < 100:
    #         train_features = pd.concat([train_features, currentFlow], axis=0)
    #     else:
    #         test_features = pd.concat([test_features, currentFlow], axis=0)
    # train_features.reset_index(inplace=True, drop=True)
    # test_features.reset_index(inplace=True, drop=True)
    #
    # labels = data_scaled[label].copy()
    # train_labels = pd.DataFrame()
    # test_labels = pd.DataFrame()
    # for jj in range(int(len(features) / 83)):
    #     currentFlow = pd.DataFrame()
    #     for ii in range(83):
    #         currentFlowNodes = labels.loc[[ii + (jj * 83)]]
    #         currentFlowNodes.reset_index(inplace=True, drop=True)
    #         currentFlowNodes.rename(columns={'demand_value': 'demand_value_' + str(ii + 1)}, inplace=True)
    #         currentFlow = pd.concat([currentFlow, currentFlowNodes], axis=1)
    #     if jj < 100:
    #         train_labels = pd.concat([train_labels, currentFlow], axis=0)
    #     else:
    #         test_labels = pd.concat([test_labels, currentFlow], axis=0)
    ...
    #
    #     featuresRow.append([data.loc[ii, 'pressure_value'], data.loc[ii, 'base_demand']])
    #     labelsRow.append(data.loc[ii, 'demand_value'])
    #
    #     if ((ii>1) and (ii%82 == 0)):
    #         features.append(featuresRow)
    #         labels.append(labelsRow)
    #         featuresRow = []
    #         labelsRow = []
    #
    ...
    #
    # train_features = features1[:100]
    # test_features = features1[100:]
    #
    # train_labels = labels1[:100]
    # test_labels = labels1[100:]


    ###########
    ########### USE DATAFRAME not shaped
    ###########
